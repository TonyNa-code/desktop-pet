...
右键桌宠，选择 `对话 > 打开聊天` 会打开小型悬浮聊天窗。选择 `对话 > 对话设置`，或在聊天窗里点 `设置`，会打开单独的设置窗口。设置窗口里填写：
...
TTS 使用系统自带语音，默认关闭。在设置窗口里把 `回复朗读` 从 `关闭朗读` 改成 `系统默认语音` 或具体语音后，保存时会自动开启朗读；保持关闭则不会朗读。桌宠收到回复时会显示更大的气泡，并根据回复内容切换到适合的表情或动作。
