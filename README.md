...
`关系语气（测试）` 是可选模块，默认关闭。开启后，可以手动设置当前数值、阶段阈值，以及低/中/高阶段分别对应的语气规则。聊天时这些规则会写入 LLM 的 system prompt；关闭后，LLM 不会收到好感度阶段，也不会按好感度调整口吻。这个功能仍在测试，如果追求稳定表现，建议保持关闭。

LLM 区域填写：

- `Base URL`：OpenAI-compatible 接口地址，例如本地模型服务 `http://localhost:11434/v1`，或云端模型平台提供的兼容地址
...
- 自定义 TTS 接口：适合网络 TTS 或自行封装的服务。应用会按 JSON 模板发送请求，模板里用 `{{text}}` 代表要朗读的文字。
...
应用会在本机保存简单状态，例如窗口设置、角色设置、人物设定、聊天配置和可选关系语气数值。
