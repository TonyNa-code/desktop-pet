...
`互动数值` 是可选模块。开启后，可以配置当前好感值、点击/聊天/双击/长按的增长幅度、增长冷却、每日增长上限、阶段阈值，以及低/中/高阶段分别对应的语气规则。聊天时这些规则会写入 LLM 的 system prompt；关闭后，LLM 不会收到好感度阶段，也不会按好感度调整口吻。
